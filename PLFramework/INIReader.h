...
inline void INIReader::Load(const std::string &filename)
{
	if (!_init) {
		_error = ini_parse(filename.c_str(), ValueHandler, this);
		_init = true;
...
inline bool INIReader::Exists(const std::string& section, const std::string& name) const
{
